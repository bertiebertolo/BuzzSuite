...
class PlotManager:
    def __init__(self, log, save_dir=None,group_colors=None, group_markers=None, category_colors=None, category_markers=None):
        self.log = log
        if save_dir is None:
            save_dir = os.path.join(get_package_root(), "plots")
    ...
    def initialize_figure(self, subplots_shape=(1, 1), figsize=None, dpi=None, tight_layout=True):
        """Initialize a matplotlib figure and axes with consistent styling."""
        if figsize is None:
            figsize = tuple(map(int, os.environ.get('BUZZSUITE_DEFAULT_FIGSIZE', '8,6').split(',')))
        if dpi is None:
            dpi = int(os.environ.get('BUZZSUITE_DEFAULT_DPI', '100'))
    ...
    def save_plot(self, fig, filename):
        ...
        pdf_dpi = int(os.environ.get('BUZZSUITE_PDF_DPI', '300'))
        fig.savefig(png_path, format='png', bbox_inches='tight')
        fig.savefig(pdf_path, format='pdf', bbox_inches='tight', dpi=pdf_dpi, transparent=False)
    ...
    def plot_entire_time_series(self, selected_vars, resample_interval, moving_avg_window, start_date_str, end_date_str, threshold, experiments_to_plot, groups_to_plot, categories_to_plot, grouped_experiments, start_hour=None, end_hour=None, normalize=False, ax=None, display_names=None, ylim=None):
        start_date = pd.to_datetime(start_date_str)
        end_date = pd.to_datetime(end_date_str)
        
        self.log(f"[DEBUG] Plot entire time series: vars={selected_vars}, exps={len(experiments_to_plot)}, groups={len(groups_to_plot)}, cats={len(categories_to_plot)}")
        self.log(f"[DEBUG] Date range: {start_date_str} to {end_date_str}")
        ...
        for exp_data in experiments_to_plot:
            ...
            for i, (var, display_name) in enumerate(zip(selected_vars, display_names)):
                ...
                if var_data.empty:
                    self.log(f"[DEBUG] {alias} - {var}: No data found")
                    continue
                
                var_data = self.crop_time(start_date, end_date, var_data)
                
                if var_data.empty:
                    self.log(f"[DEBUG] {alias} - {var}: Empty after crop_time")
                    continue

                # Skip processing for sparse event data
                if var not in ["flight_duration", "average_speed"]:
                    ...
                    # For sparse event data, plot as scatter points
                    self.log(f"[DEBUG] Plotting sparse data {alias} - {var}: {len(var_data)} points")
                    ax.scatter(var_data.index, var_data.values, label=f"{alias} - {display_name}", 
        ...
        for category_name, groups in grouped_experiments.items():
            for group_name in groups_to_plot:
                if group_name in groups:
                    ...
                    for i, (var, display_name) in enumerate(zip(selected_vars, display_names)):
                        # For sparse event data (flight_duration, average_speed), plot all individual points without averaging
                        if var in ["flight_duration", "average_speed"]:
                            # Concatenate all individual measurements from the group
                            all_event_data = pd.concat(group_data[var])
                            if len(all_event_data) > 0:
                                ax.scatter(all_event_data.index, all_event_data.values, label=f"{group_name} - {display_name}",
                                          color=color, marker=marker, s=20, alpha=0.6)
                        else:
                            # For continuous data, average across experiments
                            unified_index = pd.date_range(start=start_date, end=end_date, freq=resample_interval or 'min')
        ...
        for category_name in categories_to_plot:
            ...
            for group_name, group in grouped_experiments[category_name].items():
                ...
                # For sparse event data, just concatenate without averaging
                for var in selected_vars:
                    if var in ["flight_duration", "average_speed"]:
                        category_data[var].append(pd.concat(group_data[var]))
                    else:
                        # For continuous data, average across groups
                        all_series = [
            ...
            for i, (var, display_name) in enumerate(zip(selected_vars, display_names)):
                # For sparse event data, plot all individual points
                if var in ["flight_duration", "average_speed"]:
                    all_event_data = pd.concat(category_data[var]) if category_data[var] else pd.Series()
                    if len(all_event_data) > 0:
                        ax.scatter(all_event_data.index, all_event_data.values, label=f"{category_name} - {display_name}",
                                  color=color, marker=marker, s=20, alpha=0.6)
                else:
                    # For continuous data, calculate final average
    ...
    def plot_avg_over_days(self, selected_vars, resample_interval, moving_avg_window, start_date_str, end_date_str, threshold, experiments_to_plot, groups_to_plot, categories_to_plot, grouped_experiments, start_hour=None, end_hour=None, normalize=False, ax=None, display_names=None, ylim=None):
        start_date = pd.to_datetime(start_date_str)
        end_date = pd.to_datetime(end_date_str)
        ...
        for exp_data in experiments_to_plot:
            ...
            for i, (var, display_name) in enumerate(zip(selected_vars, display_names)):
                ...
                if normalize:
                    var_data = self.normalize_by_daily_total(var_data)
                var_data = self.crop_time(start_date, end_date, var_data).between_time(start_hour, end_hour)
        ...
        # Apply ylim if provided (overrides the bottom=0 setting above)
        if ylim and ylim != '':
            ax.set_ylim([0, np.float64(ylim)])
    ...
    def get_var_data(self, var, population_data, individual_data, threshold):
        if var in population_data.columns:
            ...
            if isinstance(threshold, dict):
                if var == "flight_duration":
                    duration_min = threshold.get("duration_min", None)
                    if duration_min is not None:
                        var_data = var_data[var_data > float(duration_min)]
                elif var == "average_speed":
                    speed_min = float(threshold.get("speed_min", 1.0))
                    speed_max = float(threshold.get("speed_max", 40.0))
                    var_data = var_data[var_data.between(speed_min, speed_max, inclusive='both')]
            elif var in ["flight_duration", "average_speed"] and threshold:
...
###################### Scatter bar plot variabiltity ###################
    def plot_scatter_variability(self, selected_vars, resample_interval, moving_avg_window, start_date_str, end_date_str, threshold, experiments_to_plot, groups_to_plot, categories_to_plot, grouped_experiments, start_hour=None, end_hour=None, ax=None, display_names=None, normalize=False, ylim=None):
        start_date = pd.to_datetime(start_date_str)
        end_date = pd.to_datetime(end_date_str)

        # Create the plot axis if none is provided
        if ax is None:
            scatter_figsize = tuple(map(int, os.environ.get('BUZZSUITE_FIGSIZE_SCATTER', '10,6').split(',')))
        ...
        for i, (var, display_name) in enumerate(zip(selected_vars, display_names)):
            try:
                ...
                for exp_data in experiments_to_plot:
                    ...
                    if normalize:
                        var_data = self.normalize_by_daily_total(var_data)
                    var_data = var_data.between_time(start_hour, end_hour)
...
###################### Dimension reduction ###############
    def visualize_reduced_data(self, reduced_data, experiments, method, days, plot_days, interval_hours, data_level, normalize, explained_variance):
        ...
        try:
            ...
            pass  # Ignore errors when closing plot windows
...
####### GLMM plotting 
    ...
    def plot_glmm_results(self, results, fixed_effects, grouped_experiments):
        ...
        glmm_figsize = tuple(map(int, os.environ.get('BUZZSUITE_FIGSIZE_GLMM_RESULTS', '14,16').split(',')))
        fig, axes = plt.subplots(nrows=4, ncols=1, figsize=glmm_figsize)
